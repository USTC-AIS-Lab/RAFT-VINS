...
calcflow_raft::calcflow_raft() {
    ...
    trace_sctipt_ = std::make_unique<Trace_script>();
...
// tuple<Tensor, Tensor> calcflow_raft::decoder_forward(Tensor& tensor0, Tensor& tensor1, Tensor& tensor2) {
//     void *buffer[5]{};
//     auto sizes_0 = tensor0.sizes();
//     auto sizes_1 = tensor1.sizes();
//     auto sizes_2 = tensor2.sizes();
//     int index0 = decoder_engine->getBindingIndex("inp");
//     int index1 = decoder_engine->getBindingIndex("corr");
//     int index2 = decoder_engine->getBindingIndex("flow");
//     buffer[index0] = tensor0.data_ptr();
//     buffer[index1] = tensor1.data_ptr();
//     buffer[index2] = tensor2.data_ptr();
//     //动态时，也需要设置输入维度
...
//     int index3 = decoder_engine->getBindingIndex("mask");
//     int index4 = decoder_engine->getBindingIndex("flow_out");
//     Tensor fmat0 = torch::zeros({1, 576, sizes_0[2], sizes_0[3]}, opt); 
//     Tensor fmat1 = torch::zeros({1, 2, sizes_0[2], sizes_0[3]}, opt); 
//     buffer[index3] = fmat0.data_ptr(); 
//     buffer[index4] = fmat1.data_ptr(); 
...
Tensor calcflow_raft::trace_predict(Tensor& img1_tensor, Tensor& img2_tensor) {
    ...
    // cout << "fmap1.sizes()" << fmap1.sizes() << endl;
    // cout << "fmap2.sizes()" << fmap2.sizes() << endl;
    // cout << "fmap1.sizes(): " << fmap1.sizes() << endl;
    // auto result_fnet = trace_sctipt_->fnet_->forward({img1_tensor, img2_tensor}).toTuple();
    // Tensor fmap1 = result_fnet->elements()[0].toTensor();
    // Tensor fmap2 = result_fnet->elements()[1].toTensor();
    // cout << "fnet_time.toc()" << trace_time.toc() << endl;

    //计算相关性correlation 
    // trace_time.tic();
    Corr(fmap1, fmap2);
    // cout << "corr_time.toc()" << trace_time.toc() << endl;

    // trace_time.tic();
    Tensor cmap = cnet_forward(img1_tensor);
    // cout << "cmap.sizes()" << cmap.sizes() << endl;
    // Tensor cmap = trace_sctipt_->cnet_->forward({img1_tensor}).toTensor();
    // cout << "cnet_time.toc()" << trace_time.toc() << endl;
    ...
    for (int i = 0; i < iters; i++) {
        ...
        // auto update_result = trace_sctipt_->decoder_->forward({inp, corr, flow}).toTuple();
        // Tensor mask = update_result->elements()[0].toTensor();
        // Tensor delta_flow = update_result->elements()[1].toTensor();

        coords_1 += delta_flow;
        if (i == iters -1)
            flow_predictions = upsample_flow(coords_1 - coords_0, mask);
            // flow_predictions = upflow8(coords_1 - coords_0);
    }
    // cout << "update_time.toc()" << trace_time.toc() << endl;
    // cout << "iter_spend_time: " << tt.toc() << endl;
