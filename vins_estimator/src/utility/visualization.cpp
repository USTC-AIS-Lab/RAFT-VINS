...
{
    pub_latest_odometry = n.advertise<nav_msgs::Odometry>("imu_propagate", 10000);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        //       << estimator.Vs[WINDOW_SIZE].z() << "," << endl; //euroc不应该有这个,号啊，但不加这个逗号还是不能用euroc评价
        // foutC.close();

        //UMA-VI数据集
        ofstream foutC(VINS_RESULT_PATH, ios::app); //不会覆盖原有内容，会继续写入
        foutC.setf(ios::fixed, ios::floatfield);
        foutC.precision(0);
        foutC << header.stamp.toSec() * 1e9 << ",";
        foutC.precision(5);
