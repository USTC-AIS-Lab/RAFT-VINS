...
Tensor make_colorwheel(){
    ...
    // static auto gpu = torch::TensorOptions(torch::kCPU);
...
cv::Mat visual_flow_image(torch::Tensor &flow_uv)
{
    ...
    //  std::cout << "visual_flow_image1:  " << visualtt.toc() << std::endl;
    // visualtt.tic();
    cv::Mat img =  cv::Mat(flow.sizes()[0],flow.sizes()[1],CV_8UC3,flow.to(torch::kCPU).data_ptr()).clone();
    // std::cout << "visual_flow_image2:  " << visualtt.toc() << std::endl;
    return img;
}